...
static char scip_checksum(const char buffer[], int32_t size)
{
...
static int32_t set_errno_and_return(urg_t *urg, int32_t urg_errno)
{
...
static int32_t scip_response(urg_t *urg, const char *command,
                             const int32_t expected_ret[], int32_t timeout,
                             char *receive_buffer, int32_t receive_buffer_max_size)
{
    char *p = receive_buffer;
    char buffer[BUFFER_SIZE];
    int32_t filled_size = 0;
    int32_t line_number = 0;
    int32_t ret = URG_UNKNOWN_ERROR;

    int32_t write_size = (int)strlen(command);
    int32_t n = connection_write(&urg->connection, command, write_size);
    ...
    do
    {
        ...
        else if (p && (line_number > 0) && (n < (receive_buffer_max_size - filled_size)))
        {
        ...
        if (line_number == 1)
        {
            ...
            {
                int32_t i;
                int32_t actual_ret = strtol(buffer, NULL, 10);
...
static void clear_urg_communication_buffer(urg_t *urg, int32_t timeout)
{
    char buffer[BUFFER_SIZE];
    int32_t n;
...
static void ignore_receive_data(urg_t *urg, int32_t timeout)
{
    char buffer[BUFFER_SIZE];
    int32_t n;
...
static void ignore_receive_data_with_qt(urg_t *urg, int32_t timeout)
{
...
static int32_t change_sensor_baudrate(urg_t *urg,
                                      int32_t current_baudrate, int32_t next_baudrate)
{
    enum
    {
        SS_COMMAND_SIZE = 10
    ...
    int32_t ss_expected[] = {0, 3, 4, EXPECTED_END};
    int32_t ret;
...
static int32_t connect_urg_device(urg_t *urg, int32_t baudrate)
{
    int32_t try_baudrate[] = {19200, 57600, 115200, 250000, 500000, 750000};
    int32_t try_times = sizeof(try_baudrate) / sizeof(try_baudrate[0]);
    int32_t i;
    ...
    for (i = 0; i < try_times; ++i)
    {
        ...
        int32_t qt_expected[] = {0, EXPECTED_END};
        ...
        if (ret > 0)
        {
            ...
            {
                int32_t scip20_expected[] = {0, EXPECTED_END};
            ...
            {
                int32_t tm2_expected[] = {0, EXPECTED_END};
...
static int32_t receive_parameter(urg_t *urg)
{
    enum
    {
    ...
    int32_t pp_expected[] = {0, EXPECTED_END};
    unsigned short received_bits = 0x0000;
    char *p;
    int32_t i;

    int32_t ret = scip_response(urg, "PP\n", pp_expected, MAX_TIMEOUT,
                                receive_buffer, RECEIVE_BUFFER_SIZE);
    if (ret < 0)
    {
        return ret;
    ...
    for (i = 0; i < (ret - 1); ++i)
    {
        ...
        {
            int32_t rpm = strtol(p + 5, NULL, 10);
...
int32_t urg_scip_decode(const char data[], int32_t size)
{
    const char *p = data;
...
static int32_t parse_parameter(const char *parameter, int32_t size)
{
    char buffer[5];
...
static int32_t receive_length_data(urg_t *urg, int32_t length[],
                                   unsigned short intensity[],
                                   urg_measurement_type_t type, char buffer[])
{
    int32_t n;
    int32_t step_filled = 0;
    int32_t line_filled = 0;
    int32_t multiecho_index = 0;

    int32_t each_size =
        (urg->received_range_data_byte == URG_COMMUNICATION_2_BYTE) ? 2 : 3;
    int32_t data_size = each_size;
    int32_t is_intensity = URG_FALSE;
    int32_t is_multiecho = URG_FALSE;
    int32_t multiecho_max_size = 1;

    if ((type == URG_DISTANCE_INTENSITY) || (type == URG_DISTANCE_INTENSITY_IO) || (type == URG_MULTIECHO_INTENSITY))
    {
        data_size *= 2;
...
static int32_t receive_data(urg_t *urg, int32_t data[], unsigned short intensity[], int32_t io[],
                            int32_t *time_stamp)
{
    ...
    int32_t ret = 0;
    int32_t n;
    int32_t extended_timeout = urg->timeout + 2 * (urg->scan_usec * (urg->scanning_skip_scan) / 1000);
...
int32_t urg_open(urg_t *urg, urg_connection_type_t connection_type,
                 const char *device_or_address, int32_t baudrate_or_port)
{
    int32_t ret;
    int32_t baudrate = baudrate_or_port;
...
void urg_set_timeout_msec(urg_t *urg, int32_t msec)
{
    urg->timeout = msec;
}

int32_t urg_start_time_stamp_mode(urg_t *urg)
{
    const int32_t expected[] = {0, EXPECTED_END};
    int32_t n;
...
int32_t urg_time_stamp(urg_t *urg)
{
    const int32_t expected[] = {0, EXPECTED_END};
    char buffer[BUFFER_SIZE];
    char *p;
...
int32_t urg_stop_time_stamp_mode(urg_t *urg)
{
    int32_t expected[] = {0, EXPECTED_END};
    int32_t n;
...
static int32_t send_distance_command(urg_t *urg, int32_t scan_times, int32_t skip_scan,
                                     char single_scan_ch, char continuous_scan_ch,
                                     char scan_type_ch)
{
    char buffer[BUFFER_SIZE];
    int32_t write_size = 0;
    int32_t front_index = urg->front_data_index;
    int32_t n;
...
int32_t urg_start_measurement(urg_t *urg, urg_measurement_type_t type, int32_t scan_times, int32_t skip_scan,
...
int32_t urg_get_distance(urg_t *urg, int32_t data[], int32_t *time_stamp)
{
    if (!urg->is_active)
    {
        return set_errno_and_return(urg, URG_NOT_CONNECTED);
...
int32_t urg_get_distance_io(urg_t *urg, int32_t data[], int32_t io[], int32_t *time_stamp)
{
    if (!urg->is_active)
    {
        return set_errno_and_return(urg, URG_NOT_CONNECTED);
...
int32_t urg_get_distance_intensity(urg_t *urg,
                                   int32_t data[], unsigned short intensity[],
                                   int32_t *time_stamp)
{
    if (!urg->is_active)
    {
...
int32_t urg_get_distance_intensity_io(urg_t *urg,
                                      int32_t data[], unsigned short intensity[], int32_t io[],
                                      int32_t *time_stamp)
{
    if (!urg->is_active)
    {
...
int32_t urg_get_multiecho(urg_t *urg, int32_t data_multi[], int32_t *time_stamp)
{
    if (!urg->is_active)
    {
        return set_errno_and_return(urg, URG_NOT_CONNECTED);
...
int32_t urg_get_multiecho_intensity(urg_t *urg,
                                    int32_t data_multi[],
...
int32_t urg_stop_measurement(urg_t *urg)
{
    enum
    {
        MAX_READ_TIMES = 3
    };
    int32_t ret = URG_INVALID_RESPONSE;
    int32_t n;
    int32_t i;
...
int32_t urg_set_scanning_parameter(urg_t *urg, int32_t first_step, int32_t last_step,
                                   int32_t skip_step)
{
...
int32_t urg_set_measurement_data_size(urg_t *urg,
...
int32_t urg_laser_on(urg_t *urg)
{
    int32_t expected[] = {0, 2, EXPECTED_END};
    int32_t ret;
...
int32_t urg_laser_off(urg_t *urg)
{
    return urg_stop_measurement(urg);
}

int32_t urg_reboot(urg_t *urg)
{
    int32_t expected[] = {0, 1, EXPECTED_END};
    int32_t ret;
    int32_t i;
...
int32_t urg_sleep(urg_t *urg)
{
    enum
    {
        RECEIVE_BUFFER_SIZE = 4
    };
    int32_t sl_expected[] = {0, EXPECTED_END};
    char receive_buffer[RECEIVE_BUFFER_SIZE + 1];

    int32_t qtResult = urg_stop_measurement(urg);
    ...
    int32_t ret = scip_response(urg, "%SL\n", sl_expected, 1000,
                                receive_buffer, RECEIVE_BUFFER_SIZE);
    if (ret < 0)
    {
        return ret;
...
int32_t urg_is_stable(urg_t *urg)
{
    const char *stat = urg_sensor_status(urg);

    return (strncmp("Stable 000 no error", stat, 19) == 0 || strncmp("Sensor works well", stat, 17) == 0 || strncmp("sensor is working normally", stat, 26) == 0 || strncmp("failed to synchronize", stat, 21) == 0 || strncmp("detected multiple master sensor", stat, 31) == 0) ? 1 : 0;
}

static char *copy_token(char *dest, char *receive_buffer,
                        const char *start_str, const char *end_ch, int32_t lines)
{
...
static const char *receive_command_response(urg_t *urg,
                                            ...
                                            const char *command,
                                            int32_t response_lines)
{
    const int32_t vv_expected[] = {0, EXPECTED_END};
    int32_t ret;
...
static int32_t receive_II_command_response(urg_t *urg,
                                           char *buffer, int32_t buffer_size)
{
    const int32_t vv_expected[] = {0, EXPECTED_END};
    int32_t ret;
