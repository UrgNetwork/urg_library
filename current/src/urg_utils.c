...
void urg_distance_min_max(const urg_t *urg,
                          int32_t *min_distance, int32_t *max_distance)
{
    if (!urg->is_active)
    {
        *min_distance = 1;
    ...
    *max_distance =
        (urg->range_data_byte == URG_COMMUNICATION_2_BYTE) ? max(urg->max_distance, 4095) : urg->max_distance;
}

void urg_step_min_max(const urg_t *urg, int32_t *min_index, int32_t *max_index)
{
    if (!urg->is_active)
    {
        *min_index = 1;
...
int32_t urg_scan_usec(const urg_t *urg)
{
    if (!urg->is_active)
    {
...
int32_t urg_max_data_size(const urg_t *urg)
{
    if (!urg->is_active)
    {
        return URG_NOT_CONNECTED;
...
double urg_index2rad(const urg_t *urg, int32_t index)
{
    ...
    // \~japanese scanning_skip_step = 0 のときは scanning_skip_step = 1 とみなす
...
double urg_index2deg(const urg_t *urg, int32_t index)
{
    return urg_index2rad(urg, index) * 180.0 / M_PI;
}

int32_t urg_rad2index(const urg_t *urg, double radian)
{
    int32_t index;
    ...
    index =
        (int32_t)(floor((urg->area_resolution * radian / (2.0 * M_PI) + 0.5))) + urg->front_data_index;

    return min(max(0, index), urg->last_data_index);
}

int32_t urg_deg2index(const urg_t *urg, double degree)
{
    return urg_rad2index(urg, degree * M_PI / 180.0);
}

int32_t urg_rad2step(const urg_t *urg, double radian)
{
    if (!urg->is_active)
    {
        return URG_NOT_CONNECTED;
...
int32_t urg_deg2step(const urg_t *urg, double degree)
{
    return urg_rad2step(urg, degree * M_PI / 180.0);
}

double urg_step2rad(const urg_t *urg, int32_t step)
{
    if (!urg->is_active)
    {
        return URG_NOT_CONNECTED;
...
double urg_step2deg(const urg_t *urg, int32_t step)
{
    return urg_step2rad(urg, step) * 180.0 / M_PI;
}

int32_t urg_step2index(const urg_t *urg, int32_t step)
{
    int32_t measure_step;
...
void urg_delay(int32_t delay_msec)
{
#if defined(URG_WINDOWS_OS)
    Sleep(delay_msec);
#else
