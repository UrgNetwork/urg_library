...
#include "urg_ring_buffer.h"
    ...
    typedef struct
    {
        ...
        int32_t current_timeout; //!< \~japanese タイムアウトの設定時間 [msec]  \~english Timeout configuration value
#else
    int32_t fd;         //!< \~japanese ファイルディスクリプタ  \~english File descriptor
    ...
    extern int32_t serial_open(urg_serial_t *serial, const char *device, int32_t baudrate);
    ...
    extern int32_t serial_set_baudrate(urg_serial_t *serial, int32_t baudrate);

    //! \~japanese データを送信する  \~english Sends data over serial connection
    extern int32_t serial_write(urg_serial_t *serial, const char *data, int32_t size);

    //! \~japanese データを受信する  \~english Gets data from serial connection
    extern int32_t serial_read(urg_serial_t *serial,
    ...
    extern int32_t serial_readline(urg_serial_t *serial,
                                   char *data, int32_t max_size, int32_t timeout);

    //! \~japanese エラー文字列を格納して返す  \~english Stores the serial error message
    extern int32_t serial_error(urg_serial_t *serial,
                                char *error_message, int32_t max_size);
